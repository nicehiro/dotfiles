...
export function assistantUsageTokens(messages: unknown[]): number {
	...
	for (const message of messages) {
		...
		// Pi reports cache-read tokens separately from input. Keep the local input + output budget.
		total += input + output;
	}
	return total;
}
...
The active thread goal has reached its token budget. No new automatic continuation will be queued. Editing or resuming does not reset the budget. Summarize progress, or use /goal clear before creating a new goal with a new budget.`;
}
