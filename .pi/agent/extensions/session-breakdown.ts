...
export function choosePaletteFromLast30Days(range30: RangeAgg, topN = 4, groupProviders = false): {
	modelColors: Map<ModelKey, RGB>;
	otherColor: RGB;
	orderedModels: ModelKey[];
} {
...
export function renderModelTable(range: RangeAgg, mode: MeasurementMode, maxRows = 8, groupProviders = false): string[] {
...
export class BreakdownComponent implements Component {
	...
	render(width: number): string[] {
		...
		// On short terminals prioritize the summary and table over the graph and legend.
		if (termRows > 0 && lines.length + 4 > termRows) lines.splice(5);
		lines.push("");
		const availableRows = termRows > 0 ? Math.max(0, termRows - lines.length) : tableLines.length;
		for (const tl of tableLines.slice(0, availableRows)) lines.push(truncateToWidth(tl, width));

		// Ensure no overly long lines (truncateToWidth already), but keep at least 1 line.
		this.cachedWidth = width;
		this.cachedRows = termRows;
		this.cachedLines = (termRows > 0 ? lines.slice(0, termRows) : lines)
